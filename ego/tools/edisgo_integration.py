...
if not "READTHEDOCS" in os.environ:
    ...
    from edisgo.grid.network import EDisGo, Results, TimeSeriesControl
...
class EDisGoNetworks:
    ...
    def _init_status(self):
        ...
        self._status_path = os.path.join(self._status_dir, self._status_file + ".csv")
    ...
    def _status_update(self, mv_grid_id, time, message=None, show=True):
        ...
        """
        status = pd.read_csv(self._status_path, index_col=0)
    ...
    def _set_scenario_settings(self):
        ...
        if (self._storage_distribution is True) & (self._ext_storage is False):
            logger.warning(
                "Storage distribution (MV grids) is active, "
    ...
    def _run_edisgo(self, mv_grid_id):
        ...
        ding0_filepath = self._ding0_files + "/ding0_grids__" + str(mv_grid_id) + ".pkl"
        ...
        edisgo_grid = EDisGo(
            ding0_grid=ding0_filepath, worst_case_analysis="worst-case"
        )
        ...
        edisgo_grid.network.config["grid_expansion_allowed_voltage_deviations"] = {
        ...
        costs = pd.DataFrame(
            ...
            index=[[edisgo_grid.network.id] * len(costs_grouped), costs_grouped.index],
        ).reset_index()
        ...
        if apply_curtailment:
            ...
            curt_cols = [
                i
                for i in specs["ren_curtailment"].columns
                if i in solar_wind_capacities.index
            ]

            if not curt_cols:
                raise ImportError(("MV grid {}: Data doesn't match").format(mv_grid_id))

            curt_abs = pd.DataFrame(columns=pd.MultiIndex.from_tuples(curt_cols))

            for col in curt_abs:
                curt_abs[col] = (
                    specs["ren_curtailment"][col] * solar_wind_capacities[col]
                )
        ...
        if costs_without_storage is not None:
            costs_with_storage = edisgo_grid.network.results.grid_expansion_costs[
                "total_costs"
            ].sum()
            if costs_with_storage >= costs_without_storage:
                logger.warning("Storage did not benefit MV grid {}".format(mv_grid_id))
                st = edisgo_grid.network.mv_grid.graph.nodes_by_attribute("storage")
                for storage in st:
    ...
    def _laod_edisgo_results(self):
        ...
        for idx, row in self._grid_choice.iterrows():
            ...
            try:
                ...
                grid_expansion_costs = pd.read_csv(file_path, index_col=0)
                ...
                s_res = pd.read_csv(pf_path, index_col=0, parse_dates=True)

                # Configs
                config_path = os.path.join(
                    self._csv_import, str(mv_grid_id), "configs.csv"
                )
                ...
                pypsa_path = os.path.join(
                    self._csv_import, str(mv_grid_id), "pypsa_network"
                )
                ...
                if os.path.exists(storage_path):
                    storages = pd.read_csv(storage_path, index_col=0)
                else:
                    storages = pd.DataFrame(columns=["nominal_power", "voltage_level"])

                edisgo_grid = _EDisGoImported(
                    grid_expansion_costs, s_res, storages, imported_pypsa, edisgo_config
                )
...
class _ETraGoData:
    ...
    def __init__(self, etrago_network):
        ...
        self.snapshots = getattr(etrago_network, "snapshots")
        self.storage_units = getattr(etrago_network, "storage_units")
        self.storage_units_t = getattr(etrago_network, "storage_units_t")
        self.generators = getattr(etrago_network, "generators")
        self.generators_t = getattr(etrago_network, "generators_t")
...
class _EDisGoImported:
    ...
    def __init__(self, grid_expansion_costs, s_res, storages, pypsa, edisgo_config):

        self.network = _NetworkImported(
            grid_expansion_costs, s_res, storages, pypsa, edisgo_config
        )
...
class _NetworkImported:
    ...
    def __init__(self, grid_expansion_costs, s_res, storages, pypsa, edisgo_config):
        ...
        self.results = _ResultsImported(grid_expansion_costs, s_res, storages)
...
class _ResultsImported:
    ...
    def __init__(self, grid_expansion_costs, s_res, storages):
