...
class EDisGoNetworks:
    ...
    def _set_scenario_settings(self):
        ...
        # Name of an edisgo.run preset (or path to a custom pipeline
        # YAML). The per-grid workflow is delegated to
        # edisgo.run.run_edisgo(); the ego default is the preset with
        # overlying-grid requirements from eTraGo.
        self._preset = self._edisgo_args.get("preset") or "overlying_grid_opf"
    ...
    def _build_run_edisgo_config(self, mv_grid_id):
        ...
        legacy = self._edisgo_args.get("legacy_ding0_grids")
        if legacy is not None:
            cfg["grid"]["legacy_ding0_grids"] = legacy
        # No database section: eDisGo auto-detects the source (egon-data
        # configuration file if present, otherwise OEP). Pin it explicitly
        # with a "database" block in the eDisGo config YAML if needed.
    ...
    def run_edisgo(self, mv_grid_id):
        ...
        Delegates the per-grid workflow to edisgo.run.run_edisgo via
        :meth:`_run_one_grid_via_runner`.
        """
